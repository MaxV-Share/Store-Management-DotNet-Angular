...
import {
  IBaseLoading,
  IBasePaging,
  ICategoryAddOrUpdateModel,
  // ICategoryDataTable,
  ICategoryDetailModel,
  ICategoryModel,
  IFilterBodyRequest,
  IPagination,
} from "models";
...
export interface CategoryState {
  isLoading: boolean;
  // table: IBaseLoading & ICategoryDataTable;
  table: any;
...
const categorySlice = createSlice({
  ...
  reducers: {
    ...
    fetchCategoriesSuccess(
      state,
      action: PayloadAction<IBasePaging<ICategoryDetailModel>>
    ) {
    ...
    setFilter(state, action: PayloadAction<IKeyValue>) {
      if (
        get(state.filterCategoryRequest, action.payload.key) !=
    ...
    addOrUpdateCategory(
      state,
      action: PayloadAction<ICategoryAddOrUpdateModel>
    ) {
      state.categoryAddOrUpdate.isLoading = true;
    },
    addOrUpdateCategorySuccess(state, action: PayloadAction<any>) {
...
export const selectLangs = (state: RootState) => state.global.langs;
export const selectCategoryTableLoading = (state: RootState) =>
  state.category.isLoading;
export const selectCategoryTablePagination = (state: RootState) =>
  state.category.table.pagination;
export const selectFilterCategoryRequest = (state: RootState) =>
  state.category.filterCategoryRequest;
export const selectCategoryAddOrUpdate = (state: RootState) =>
  state.category.categoryAddOrUpdate;
